...
struct HfCorrelatorDplusHadrons {
  ...
  Configurable<double> cutTrackEtaMax{"cutTrackEtaMax", -1.,
                                      "max. eta of tracks"};
  ...
  void init(o2::framework::InitContext&)
  {
    ...
    registry.add("hMassDplus_2D",
                 ...
                 "(GeV/#it{c}^{2});entries",
                 {HistType::kTH2F,
                  {{massAxisBins, massAxisMin, massAxisMax},
                   {vbins, "#it{p}_{T} (GeV/#it{c})"}}});
    registry.add("hMassDplusData",
                 "Dplus candidates;inv. mass (K^{-}#pi^{+}#pi^{+}) "
                 "(GeV/#it{c}^{2});entries",
                 {HistType::kTH1F, {{massAxisBins, massAxisMin, massAxisMax}}});
    registry.add("hMassDplusMCRec",
                 "Dplus candidates;inv. mass (K^{-}#pi^{+}#pi^{+}) "
                 "(GeV/#it{c}^{2});entries",
                 {HistType::kTH1F, {{massAxisBins, massAxisMin, massAxisMax}}});
    registry.add("hMassDplusMCRecSig",
                 "Dplus signal candidates - MC reco;inv. mass (#pi K) "
                 "(GeV/#it{c}^{2});entries",
                 {HistType::kTH2F,
                  {{massAxisBins, massAxisMin, massAxisMax},
                   {vbins, "#it{p}_{T} (GeV/#it{c})"}}});
    registry.add("hMassDplusMCRecBkg",
                 "Dplus background candidates - MC reco;inv. mass (#pi K) "
                 "(GeV/#it{c}^{2});entries",
                 {HistType::kTH2F,
                  {{massAxisBins, massAxisMin, massAxisMax},
                   {vbins, "#it{p}_{T} (GeV/#it{c})"}}});
    registry.add("hcountDplustriggersMCGen",
                 "Dplus trigger particles - MC gen;;N of trigger Dplus",
                 {HistType::kTH2F,
                  {{1, -0.5, 0.5}, {vbins, "#it{p}_{T} (GeV/#it{c})"}}});
  }
    ...
    soa::Join<aod::HfCandProng3, aod::HFSelDplusToPiKPiCandidate> const& candidates)
  {
    ...
    if (collision.numContrib() > 1) {
      for (const auto& track : tracks) {
        ...
        if (std::abs(track.dcaXY()) > cutDCAxyMax ||
            std::abs(track.dcaZ()) > cutDCAzMax) {
          continue;
        }
        nTracks++;
      }
    ...
    for (auto& candidate1 : selectedDPlusCandidates) {
      ...
      registry.fill(HIST("hMassDplus_2D"), InvMassDPlus(candidate1),
                    candidate1.pt(), efficiencyWeight);
      registry.fill(HIST("hMassDplusData"), InvMassDPlus(candidate1),
                    efficiencyWeight);
      ...
      for (const auto& track : tracks) {
        ...
        if ((candidate1.index0Id() == track.mRowIndex) ||
            (candidate1.index1Id() == track.mRowIndex) ||
            (candidate1.index2Id() == track.mRowIndex)) {
          continue;
        }
        entryDplusHadronPair(getDeltaPhi(track.phi(), candidate1.phi()),
                             track.eta() - candidate1.eta(), candidate1.pt(),
                             track.pt());
  ...
  /// Dplus-Hadron correlation pair builder - for MC reco-level analysis
  /// (candidates matched to true signal only, but also the various bkg sources
  /// are studied)
  Partition<soa::Join<aod::HfCandProng3, aod::HFSelDplusToPiKPiCandidate,
                           ...
                           aod::HfCandProng3MCRec> const& candidates)
  {
    ...
    for (auto& candidate1 : recoFlagDPlusCandidates) {
      ...
      if (std::abs(candidate1.flagMCMatchRec()) ==
        ...
        registry.fill(HIST("hSelectionStatusMCRec"),
                      candidate1.isSelDplusToPiKPi());
      }
      // fill invariant mass plots from Dplus signal and background candidates
      registry.fill(HIST("hMassDplusMCRec"), InvMassDPlus(candidate1),
                    efficiencyWeight);
      if (candidate1.flagMCMatchRec() ==
          1 << DecayType::DPlusToPiKPi) { // also matched as Dplus
        registry.fill(HIST("hMassDplusMCRecSig"), InvMassDPlus(candidate1),
                      candidate1.pt(), efficiencyWeight);
      } else {
        registry.fill(HIST("hMassDplusMCRecBkg"), InvMassDPlus(candidate1),
                      candidate1.pt(), efficiencyWeight);
      }
      ...
      for (const auto& track : tracks) {
        ...
        entryDplusHadronPair(getDeltaPhi(track.phi(), candidate1.phi()),
                             track.eta() - candidate1.eta(), candidate1.pt(),
                             track.pt());
        entryDplusHadronRecoInfo(InvMassDPlus(candidate1),
                                 (int)flagDplusSignal);
  ...
  PROCESS_SWITCH(HfCorrelatorDplusHadrons, processMcRec, "Process MC Reco mode",
                 true);
                    ...
                    MCParticlesPlus3Prong const& particlesMC)
  {
    ...
    for (auto& particle1 : particlesMC) {
      ...
      if (std::abs(particle1.pdgCode()) !=
          pdg::Code::kDPlus) { // just checking the particle PDG, not the decay
                               // channel (differently from Reco: you have a BR
                               // factor btw such levels!)
        continue;
      }
      registry.fill(
      ...
      for (auto& particle2 : particlesMC) {
        ...
        if ((std::abs(particle2.pdgCode()) != 11) &&
            (std::abs(particle2.pdgCode()) != 13) &&
            (std::abs(particle2.pdgCode()) != 211) &&
            (std::abs(particle2.pdgCode()) != 321) &&
            (std::abs(particle2.pdgCode()) != 2212)) {
          continue;
        }
        entryDplusHadronPair(getDeltaPhi(particle2.phi(), particle1.phi()),
                             particle2.eta() - particle1.eta(), particle1.pt(),
                             particle2.pt());
  ...
  PROCESS_SWITCH(HfCorrelatorDplusHadrons, processMcGen, "Process MC Gen mode",
                 false);
};
